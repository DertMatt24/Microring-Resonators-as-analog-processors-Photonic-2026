classdef graph_drawer
    ...
    methods (Static)
        ...
        % Overlay the drop-port spectra of orders 1..N against the ideal
        % ODE response of the highest order.
        function spectrum_orders(Df, H_by_ord, H_ideal, IN_ring, fsr, range)
            hold on; grid on; box on;
            N = numel(H_by_ord);
            cmap = lines(N);
            leg = cell(1, N + 2);
            ...
            for n = 1:N
                H = H_by_ord{n};
                plot(Df/fsr, 10*log10(abs(H./max(abs(H))).^2), ...
                    'Color', cmap(n,:), 'LineWidth', 2);
                leg{n} = sprintf('MRR order %d', n);
            end
            plot(Df/fsr, 10*log10(abs(H_ideal./max(abs(H_ideal))).^2), ...
                'k--', 'LineWidth', 2);
            plot(Df/fsr, 10*log10(abs(IN_ring./max(abs(IN_ring))).^2), ...
                'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
            leg{N+1} = sprintf('Ideal ODE (order %d)', N);
            leg{N+2} = 'Input spectrum';

            legend(leg, 'Location', 'southwest', 'FontSize', 9);
            set(gca, 'fontsize', 12);
            ylim([-60 2]); xlim([-range range]);
            xlabel('Frequency / FSR');
            ylabel('Spectrum [dB]');
            title('Drop-port spectra vs ODE order');
        end

        % Power loss (dB) as a function of ODE order.
        function plot_power_vs_order(orders, loss_s,loss_sg)
            figure; hold on; grid on; box on;
            plot(orders, loss_s,  'b-o', 'LineWidth', 2, 'MarkerFaceColor', 'w', 'MarkerSize', 7);
            plot(orders, loss_sg, 'r-s', 'LineWidth', 2, 'MarkerFaceColor', 'w', 'MarkerSize', 7);
            set(gca, 'fontsize', 12);
            xlabel('ODE order N');
            ylabel('Power loss [dB]');
            title('Insertion loss vs cascade order');
            legend('Gaussian', 'Super-Gaussian', 'Location', 'southwest');
            xlim([min(orders)-0.3, max(orders)+0.3]);
            xticks(orders);
            hold off;
        end

        % RMSE (scaled output vs ideal ODE) as a function of ODE order.
        function plot_rmse_vs_order(orders, rmse_g, rmse_sg)
            figure; hold on; grid on; box on;
            plot(orders, rmse_g,  'b-o', 'LineWidth', 1.5, ...
                'MarkerFaceColor', 'w', 'MarkerSize', 6);
            plot(orders, rmse_sg, 'r-s', 'LineWidth', 1.5, ...
                'MarkerFaceColor', 'w', 'MarkerSize', 6);
            set(gca, 'fontsize', 12);
            xlabel('ODE order N');
            ylabel('RMSE (normalized)');
            title('Computing deviation vs cascade order');
            legend('Gaussian', 'Super-Gaussian', 'Location', 'northwest');
            xlim([min(orders)-0.5, max(orders)+0.5]);
            hold off;
        end
    end
end